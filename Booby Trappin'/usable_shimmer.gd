...
## Fresh round: clear trap/used flags so P1 can set traps again.
...
static func reset_all_for_new_round() -> void:
	p2_world_uses_exhausted = false
	var tree := Engine.get_main_loop() as SceneTree
	if tree == null:
		return
	for sprite in tree.get_nodes_in_group(GROUP):
		var n = sprite.get_node_or_null("UsableShimmer")
		if n and n.has_method("reset_for_new_round"):
			n.reset_for_new_round()
