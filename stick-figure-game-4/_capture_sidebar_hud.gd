...
func _run() -> void:
	...
	print("[CAPTURE] inv slots=", tm._inv_slots.size())
	if not sidebar.has_node("TrapCounterPanel") or not sidebar.has_node("StatsPanel") or not sidebar.has_node("InventoryPanel"):
		push_error("[CAPTURE] FAIL: expected TrapCounterPanel + StatsPanel + InventoryPanel")
		quit(FAIL)
		return
