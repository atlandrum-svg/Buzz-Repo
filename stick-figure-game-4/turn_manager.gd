...
func _default_effect_texture(id: String) -> Texture2D:
	...
	match id:
		...
		_:
			return null
	for path in paths:
		if path is String and not String(path).is_empty() and ResourceLoader.exists(String(path)):
			return load(String(path)) as Texture2D
	return null
...
func _update_held_items_visibility() -> void:
	if _inv_panel == null:
		return
	# P2 only, and only while carrying something.
	_inv_panel.visible = current_turn == "Player2" and _inv.size() > 0
